...
class SellerUpdatedLogo extends StatelessWidget {
  const SellerUpdatedLogo({super.key,required this.logo});
  final String logo;
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<UpdateSellerShopCubit, UpdateSellerShopStateModel>(
      builder: (_, state) {
        print('Imagesss ${state.logo}');
        final errorState = state.logo;
        final captureImage = state.logo.isNotEmpty
            ?state.logo: RemoteUrls.imageUrl(logo);
        return Container(
          height: 180.0,
